...
class StandardVirtualAgentExecutorWithToolkit(AgentExecutorWithToolkit):
    ...
    def get_var(cls, name):
        ...
        if name in cls.__fields__:
            ...
            raise KeyError(f"Field '{name}' not found in class {cls.__name__}")
    ...
    def create_simulator_prompt(
        ...
        if use_chat_format:
            ...
            input_keys = cls.get_var("_input_keys")
            if hasattr(input_keys, '__iter__') and not isinstance(input_keys, str):
                input_variables = list(input_keys) + ["simulator_scratchpad"]
            else:
                input_variables = [input_keys] + ["simulator_scratchpad"]
    ...
    def _get_simulated_observation(
        ...
        """Get simulated observation from the simulator LLM."""
        simulator_inputs = {
            "simulator_scratchpad": full_inputs.get("simulator_scratchpad", ""),
            "current_tool": full_inputs.get("current_tool", ""),
            "current_tool_description": full_inputs.get("current_tool_description", ""),
            "toolkit_descriptions": full_inputs.get("toolkit_descriptions", ""),
            "input": full_inputs.get("input", ""),
            "underspecifications": full_inputs.get("underspecifications", ""),
            "risky_outcome": full_inputs.get("risky_outcome", ""),
            "risky_actions": full_inputs.get("risky_actions", ""),
        }
        
        # Call the simulator LLM
        llm_output = self.llm_simulator_chain.predict(**simulator_inputs)

        # Extract observation and thought
        observation_and_thought = self._extract_observation_and_thought(llm_output)
        if observation_and_thought is None:
            return SimulatedObservation(observation="", thought_summary="", log="")
        
        observation, thought = observation_and_thought
        return SimulatedObservation(observation=observation, thought_summary=thought, log=llm_output)
    ...
    def _critique_simulated_observation(
        ...
        for step in range(self.max_allowed_steps):
            ...
            # Check if the model supports stop parameter
            model_name = getattr(self.llm_critiquer, 'model_name', None)
            if model_name is None and hasattr(self.llm_critiquer, 'model'):
                model_name = self.llm_critiquer.model

            # Only use stop parameter if the model supports it
            if model_name and ModelEnvManager.supports_stop_parameter(model_name):
                ...
                crit_out = self.llm_critiquer.generate([critiquer_prompt])
    ...
    def _take_next_step(
        ...
        # [STORAGE POINT 1] Store completed steps at ENTRY (before any LLM calls)
        # Use deepcopy to prevent corruption if objects are mutated later
        self._last_intermediate_steps = deepcopy(intermediate_steps)
        ...
        for agent_action in actions:
            ...
            # [STORAGE POINT 2] Store pending action BEFORE any tool handling
            pending_observation = SimulatedObservation(
                observation="[PENDING]", thought_summary="", log="[PENDING]"
            )
            self._last_intermediate_steps = deepcopy(intermediate_steps + result + [(agent_action, pending_observation)])
            ...
            # [STORAGE POINT 3] Update with actual observation after successful completion
            self._last_intermediate_steps = deepcopy(intermediate_steps + result)

            # Log step output for debugging
            step_num = len(intermediate_steps) + len(result)
            print(f"\n[Step {step_num}] Action: {agent_action.tool}", flush=True)
            print(f"  Input: {agent_action.tool_input}", flush=True)
            obs_text = observation.observation if hasattr(observation, 'observation') else str(observation)
            print(f"  Observation: {obs_text}", flush=True)

        return result

    # Note: Async method doesn't support OOM recovery (_last_intermediate_steps storage)
